        ...
        <div class="p-5 flex flex-col gap-10">
            <section class="flex justify-between p-10 bg-gradient-to-r from-white/20 to-transparent rounded-box shadow-2xl">
                <div class="flex items-center gap-5 text-white self-center">
                    <img src="${
											album.image
										}" alt="" class="max-w-[200px] h-[200px] object-cover object-center rounded-xl shadow-2xl">
                    <div class="flex flex-col gap-2 text-base-content">
                        <h1 class="text-6xl font-bold mb-3">${album.title}</h1>
                        <a href="/#/artist/${album.artist._id}" class="font-medium text-2xl hover:link">${
			album.artist.name
		}</a>
                ...
                <label class="self-end flex items-center gap-5">
                    ...
                    <label class="swap btn btn-primary btn-circle text-xl">
                        <input type="checkbox" id="toggle-like-album" data-album="${album._id}" ${
			isLikedAlbum ? "checked" : ""
		}>
                        <div class="swap-off">
	...
	handleEvents() {
		...
		if (toggleLikeAlbum)
			toggleLikeAlbum.onchange = async (e) => {
				...
				e.target.checked
					? toast("success", "Added this album to your library!")
					: toast("info", "Removed this album from your library!");
			};
	},
};
