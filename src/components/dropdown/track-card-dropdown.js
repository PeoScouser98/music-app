...
const trackCardDropdown = {
    render(track) {
        ...
        if (auth?.id && track?.followers)
            isLiked = track.followers.includes(auth.id)
        const toggleLikeBtn = /* html */ `
        <li>
            <label class="swap place-content-start w-full">
                <input type="checkbox" class="dropdown-like-toggle" data-track="${track._id}" ${isLiked ? "checked" : ""}/>
        ...
        const removeFromPlaylistBtn = playlistId !== null ? /* html */ `
    ...
    `
        ...
        //#region 
        const auth = storage.get("auth")
        const addPlaylistBtns = $$(".add-playlist__btn");
        const addPlaylistSubmitBtns = $$(".add-to-list--btn");
        if (addPlaylistBtns && addPlaylistSubmitBtns) {
            addPlaylistBtns.forEach((button) => {
                // if user do not login -> show warning
                if (!auth)
                    button.onclick = () => {
                        toast("info", "You haven't login yet");
                    }
                else {
                    // get the song want to add to playlist
                    button.onclick = () => {
                        const trackId = button.dataset.track;
                        if (addPlaylistSubmitBtns) addPlaylistSubmitBtns.forEach((btn) => (btn.dataset.track = trackId));
                    }
                    // add to the selected playlists
                    addPlaylistSubmitBtns.forEach((button) =>
                        button.onclick = ("click", async () => {
                            await Track.addToPlaylist(button.dataset.playlist, button.dataset.track);
        ...
        likeToggleInput.forEach(toggle => toggle.onchange = async () => {
            if (toggle) {
                if (toggle.dataset.track == audio.dataset.current)
